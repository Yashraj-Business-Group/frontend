import { Phone } from 'lucide-react';

const Footer: React.FC = () => {
  return (
    <footer className="w-full py-16 px-12 grid grid-cols-1 md:grid-cols-3 gap-12 bg-[#002451] dark:bg-black text-white">
      <div>
        <div className="text-2xl font-black text-white mb-6 uppercase font-headline">YASHRAJ GROUP</div>
        <p className="text-slate-400 text-sm leading-relaxed mb-8 max-w-xs">
          A multi-dimensional business group dedicated to providing excellence in security, facility management, and tactical corporate solutions.
        </p>
        <div className="flex gap-4">
          <a title="WhatsApp Bot" className="w-10 h-10 bg-white/10 flex items-center justify-center hover:bg-[#E51937] hover:text-white transition-colors" href="https://wa.me/918090785907" target="_blank" rel="noopener noreferrer">
            <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
              <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51a12.8 12.8 0 0 0-.57-.01c-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413Z" />
            </svg>
          </a>
          <a title="Facebook" className="w-10 h-10 bg-white/10 flex items-center justify-center hover:bg-[#E51937] hover:text-white transition-colors" href="https://www.facebook.com/YBGPVTLTD/" target="_blank" rel="noopener noreferrer">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 2h-3a5 5 0 0 0-5 5v3H7v4h3v8h4v-8h3l1-4h-4V7a1 1 0 0 1 1-1h3z" /></svg>
          </a>
          <a title="Instagram" className="w-10 h-10 bg-white/10 flex items-center justify-center hover:bg-[#E51937] hover:text-white transition-colors" href="https://www.instagram.com/yashraj.business.group?igsh=MTJ2ZXBrc2hsbjVjZw==" target="_blank" rel="noopener noreferrer">
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="2" width="20" height="20" rx="5" ry="5" /><path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" /><line x1="17.5" y1="6.5" x2="17.51" y2="6.5" /></svg>
          </a>
          <a title="Contact Number" className="w-10 h-10 bg-white/10 flex items-center justify-center hover:bg-[#E51937] hover:text-white transition-colors" href="tel:+918090785907">
            <Phone size={16} />
          </a>
        </div>
      </div>
      <div>
        <h5 className="font-headline font-bold text-white mb-8 uppercase tracking-widest text-sm">Internal Access</h5>
        <nav className="flex flex-col gap-4">
          <a className="text-white underline text-sm" href="/">Home</a>
          <a className="text-slate-400 hover:text-white transition-colors text-sm" href="/company-profile">Company Profile</a>
          <a className="text-slate-400 hover:text-white transition-colors text-sm" href="/privacy-policy">Privacy Policy</a>
          <a className="text-slate-400 hover:text-white transition-colors text-sm" href="/terms-of-service">Terms of Service</a>
          <a className="text-slate-400 hover:text-white transition-colors text-sm" href="/sitemap">Sitemap</a>
          <a className="text-slate-400 hover:text-white transition-colors text-sm" href="/admin/login">Admin Portal</a>
        </nav>
      </div>
      <div>
        <h5 className="font-headline font-bold text-white mb-8 uppercase tracking-widest text-sm">Corporate Office, Pune</h5>
        <p className="text-slate-400 text-sm leading-relaxed mb-4">
          27, Shree Ganesh Galaxy Complex,<br />
          Alandi Road (PCMC), Pune - 411105
        </p>
        <p className="text-slate-400 text-sm mb-4">
          Contact: <a href="tel:+918090785907" className="hover:text-white transition-colors">(+91) 809-078-5907</a>
        </p>
        <p className="text-slate-400 text-sm">
          Email: <a href="mailto:info@yashrajbusinessgroup.com" className="hover:text-white transition-colors">info@yashrajbusinessgroup.com</a>
        </p>
      </div>
      <div className="md:col-span-3 pt-12 border-t border-white/10 text-center text-slate-500 text-xs tracking-widest uppercase">
        © 2024 Yashraj Business Group Pvt. Ltd. All Rights Reserved.
      </div>
    </footer>
  );
};

export default Footer;
