...
const Home: React.FC = () => {
  ...
  // Lock body scroll when lightbox or testimonial sheet is open (iOS-safe)
  useEffect(() => {
    const isOpen = lightboxImage !== null || activeTestimonial !== null;
  ...
  }, [lightboxImage, activeTestimonial]);
    ...
    <main>
      ...
      <section className="py-20 md:py-32 px-5 md:px-8 bg-surface-container-low">
        <div className="max-w-7xl mx-auto">
            ...
            {[
              ...
              return (
                <div
                  ...
                  className="group bg-white rounded-2xl p-6 lg:p-8 shadow-sm border border-slate-100 flex flex-col hover:shadow-2xl hover:shadow-blue-600/20 hover:border-transparent hover:-translate-y-2 transition-all duration-500 relative overflow-hidden cursor-pointer md:cursor-default"
                >
                  {/* Vibrant professional gradient that fades in on hover */}
                  <div className="absolute inset-0 bg-gradient-to-br from-blue-600 to-primary opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none z-0"></div>

                  {/* Decorative glowing orb */}
                  <div className="absolute top-0 right-0 w-32 h-32 bg-white/20 rounded-full blur-2xl -mr-16 -mt-16 transition-opacity duration-500 opacity-0 group-hover:opacity-100 pointer-events-none z-0"></div>

                  <div className="flex items-center gap-4 mb-5 relative z-10">
                    <div className={`w-12 h-12 rounded-full ${test.color} text-white flex items-center justify-center font-bold text-xl ring-4 ring-transparent group-hover:ring-white/30 group-hover:scale-110 transition-all duration-500`}>
                      {test.avatar}
                    </div>
                    <div>
                      <div className="font-bold text-slate-800 group-hover:text-white transition-colors duration-300">{test.author}</div>
                      <div className="text-xs text-slate-500 group-hover:text-blue-200 transition-colors duration-300 flex items-center gap-1 mt-0.5">
                        {test.time} on
                        <svg className="w-3.5 h-3.5 ml-0.5 opacity-100 group-hover:brightness-200 transition-all duration-300" viewBox="0 0 24 24">
                  ...
                  <div className="flex gap-0.5 mb-4 text-amber-400 relative z-10">
                    {[...Array(test.rating)].map((_, i) => (
                      <span key={i} className="material-symbols-outlined text-[18px] group-hover:scale-110 group-hover:drop-shadow-md transition-all duration-300" style={{ fontVariationSettings: "'FILL' 1", transitionDelay: `${i * 50}ms` }}>star</span>
                    ))}
                  </div>
                  <p className="text-slate-600 text-sm flex-grow leading-relaxed relative z-10 group-hover:text-blue-50 transition-colors duration-300">"{test.quote}"</p>

                  {/* Faint large quote mark in the background */}
                  <div className="absolute -bottom-6 right-2 text-9xl text-slate-100 font-serif leading-none group-hover:text-white/10 group-hover:-translate-y-2 group-hover:-rotate-6 transition-all duration-500 pointer-events-none select-none z-0">
                    "
                  </div>

                  {/* Mobile tap hint */}
                  <div className="mt-4 flex items-center gap-1 text-xs text-slate-400 font-medium md:hidden relative z-10">
                    <span className="material-symbols-outlined text-sm">touch_app</span> Tap to read full review
                  </div>
                </div>
              )
            })}
          ...
          {/* Mobile Testimonial Bottom Sheet */}
          {activeTestimonial !== null && (
            <>
              {/* Backdrop */}
              <div
                className="fixed inset-0 z-[90] bg-slate-900/60 backdrop-blur-sm md:hidden"
                onClick={() => setActiveTestimonial(null)}
              />
              {/* Sheet */}
              <div className="fixed bottom-0 left-0 right-0 z-[91] md:hidden bg-white rounded-t-3xl shadow-2xl p-6 pb-10 animate-in slide-in-from-bottom duration-300 max-h-[85dvh] overflow-y-auto">
                {/* Drag handle */}
                ...
                {[{
                  ...
                  time: "2 months ago",
                  rating: 5,
                  fullQuote: "We are pleased to share that YBG Security has successfully obtained the PASARA license. The entire process was completed in a smooth, systematic, and professional manner. All required documentation and legal formalities were handled efficiently. The team ensured proper compliance with all government regulations and maintained transparency throughout the process. Because of their dedication and coordination, the PASARA license was obtained without any complications. Thanks to TAXWAY CONSULTANCY and Mr. Madhuri Jadhav for such a wonderful support & his professional guidance. We are proud of this achievement and remain committed to maintaining high standards of professionalism and lawful operations in the security services industry.",
                  avatar: "P",
                  color: "bg-purple-600"
                }, {
                  author: "Sanjay Kumar",
                  time: "5 months ago",
                  rating: 5,
                  fullQuote: "Best security services in Pune. Very professional and well trained guards. Management is very responsive and helpful for society requirements.",
                  avatar: "S",
                  color: "bg-blue-600"
                }, {
                  author: "Amit Sharma",
                  time: "8 months ago",
                  rating: 5,
                  fullQuote: "Excellent facility management and housekeeping services. They maintain our corporate office impeccably. Highly recommended for commercial spaces.",
                  avatar: "A",
                  color: "bg-emerald-600"
                }].filter((_, i) => i === activeTestimonial).map((test) => (
                  <div key={test.author}>
                    <div className="flex items-center gap-4 mb-5">
                      <div className={`w-14 h-14 rounded-full ${test.color} text-white flex items-center justify-center font-bold text-2xl shrink-0`}>
                        {test.avatar}
                      </div>
                      <div>
                        <div className="font-bold text-slate-800 text-lg">{test.author}</div>
                    ...
                    <p className="text-slate-700 text-sm leading-relaxed">"{test.fullQuote}"</p>
                  </div>
                ))}
              </div>
            </>
      ...
      {lightboxImage && (
        <div
          className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-slate-900/95 backdrop-blur-sm animate-in fade-in duration-300"
        ...
        >
          ...
          <div
            className="relative animate-in zoom-in-95 duration-300 cursor-default flex items-center justify-center w-full h-full px-4 pt-16 pb-6 md:p-16"
